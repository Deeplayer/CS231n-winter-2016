import numpy as np
import optim
from coco_utils import sample_coco_minibatch, fc_coco_minibatch
...
class CaptioningSolver(object):
  ...
  def _step(self):
    ...
    for p, w in self.model.params.iteritems():
      dws += np.sum(grads[p]**2)

    dws = np.sqrt(dws)
    scale_factor = 1.0
    if dws > 5.0:
      scale_factor = 5.0 / dws

    # Perform a parameter update
    for p, w in self.model.params.iteritems():
      dw = grads[p] * scale_factor
  ...
  def check_accuracy(self, X, y, num_samples=None):
    ...
    for i in xrange(N):
      ...
      c_true = y[i]
      count = 0
      for j in xrange(len(c_true)):
        if c_true[j] == 0:
          count += 1
        c_true_short = c_true[:len(c_true)-count]
      L = len(c_true_short)
      capt_short = capts[:L]
      acc.append(np.mean(capt_short == c_true_short))
  ...
  def check_BLEU(self, X, y, num_samples=None, batch_size=100):
  ...
  def train(self):
    ...
    for t in xrange(num_iterations):
      ...
      #captions, features, idxs = fc_coco_minibatch(self.data, batch_size=self.batch_size, split='train')
      #captions_val, features_val, idxs_val = fc_coco_minibatch(self.data, batch_size=self.batch_size, split='val')
      captions, features, urls = sample_coco_minibatch(self.data, batch_size=self.batch_size, split='train')
      captions_val, features_val, urls_val = sample_coco_minibatch(self.data, batch_size=self.batch_size, split='val')

      first_it = (t == 0)
      last_it = (t == num_iterations - 1)
      if first_it or last_it or epoch_end:
        train_acc = self.check_accuracy(features, captions, num_samples=50000)
        val_acc = self.check_accuracy(features_val, captions_val, num_samples=50000)
