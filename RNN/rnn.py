...
class CaptioningRNN(object):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
                 hidden_dim=128, cell_type='rnn', reg=0.0, dtype=np.float32):
        ...
        self.params['b_proj'] = np.zeros(hidden_dim)                        # [1, H]
        ...
        self.params['b_vocab'] = np.zeros(vocab_size)                       # [1, V]
    ...
    def loss(self, features, captions, mode=None):
        """
        ...
        word_vectors, word_cache = word_embedding_forward(captions_in, W_embed)   # [N, T-1, W]
        if self.cell_type == 'rnn':
            hidden, rnn_cache = rnn_forward(word_vectors, imf2hid, Wx, Wh, b)    # [N, T-1, H]
        else:
            hidden, lstm_cache = lstm_forward(word_vectors, imf2hid, Wx, Wh, b)
        scores, h2v_cache = temporal_affine_forward(hidden, W_vocab, b_vocab)    # [N, T-1, V]

        if mode == 'test':
            return scores

        loss, dscores = temporal_softmax_loss(scores, captions_out, mask)
        reg_loss = 0.5 * self.reg * sum(np.sum(W * W) for W in [W_proj, W_embed, W_vocab])
        loss += reg_loss
        ...
        grads['W_vocab'] += self.reg * W_vocab
        grads['W_embed'] += self.reg * W_embed
        grads['W_proj'] += self.reg * W_proj
        grads['Wx'] += self.reg * Wx
        grads['Wh'] += self.reg * Wh
    ...
    def sample(self, features, max_length=30):
        ...
        transform to the input image features, and the initial word is the <START> token.
