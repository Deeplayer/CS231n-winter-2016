    ...
    def loss(self, X, y=None):
        ...
        for i in xrange(t):
            ...
            if self.use_batchnorm:
                ...
                bn[i], cache2[i], batch_norm = batchnorm_forward(h[i], gamma, beta, self.bn_params[i], self.batchnorm, i, t)
                self.batchnorm[i] = batch_norm[i]
                self.batchnorm[i+t] = batch_norm[i+t]
