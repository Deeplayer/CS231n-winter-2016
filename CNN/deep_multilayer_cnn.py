...
class MultiLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=(64, 128, 192, 256),
        ...
        self.params['W10'] = np.sqrt(2.0/(filter_size**2*num_filters[3])) * np.random.randn(num_filters[3], num_filters[2], filter_size, filter_size)
        self.params['b10'] = np.zeros((1, num_filters[3]))
        ...
        self.params['W13'] = np.sqrt(2.0/num_filters[3]) * np.random.randn(num_filters[3], hidden_dim)
        self.params['b13'] = np.zeros((1, hidden_dim))
        self.params['W14'] = np.sqrt(2.0/hidden_dim) * np.random.randn(hidden_dim, num_classes)
        self.params['b14'] = np.zeros((1, num_classes))
        ...
        self.params['gamma13'] = np.ones((1, hidden_dim))
        self.params['beta13'] = np.zeros((1, hidden_dim))

        self.bn_params = []
        self.bn_params = [{'mode': 'train'} for i in xrange(13)]
    ...
    def loss(self, X, y=None):
        ...
        gamma13, beta13 = self.params['gamma13'], self.params['beta13']
        ...
        W14, b14 = self.params['W14'], self.params['b14']
        ...
        cds, cache_cds = dropout_forward(a4, self.dropout_param[1])      # convdrop2  0.4
        afs, cachefs = conv_forward_fast(cds, W3, b3, conv_param)
        bnfs, cache_bnfs = spatial_batchnorm_forward(afs, gamma3, beta3, self.bn_params[2])
        af5, cachef5 = relu_forward(bnfs)
        af, cachef = max_pool_forward_fast(af5, pool_param)

        af1, cachef1 = conv_forward_fast(af, W4, b4, conv_param)
        bnf, cache_bnf = spatial_batchnorm_forward(af1, gamma4, beta4, self.bn_params[3])
        af2, cachef2 = relu_forward(bnf)
        cd2, cache_cd2 = dropout_forward(af2, self.dropout_param[1])     # convdrop3  0.4
        a5, cache5 = conv_forward_fast(cd2, W5, b5, conv_param)
        bn3, cache_bn3 = spatial_batchnorm_forward(a5, gamma5, beta5, self.bn_params[4])
        a6, cache6 = relu_forward(bn3)
        cdf, cache_cdf = dropout_forward(a6, self.dropout_param[1])      # convdrop4  0.4
        af3, cachef3 = conv_forward_fast(cdf, W6, b6, conv_param)
        bnf2, cache_bnf2 = spatial_batchnorm_forward(af3, gamma6, beta6, self.bn_params[5])
        af4, cachef4 = relu_forward(bnf2)
        a7, cache7 = max_pool_forward_fast(af4, pool_param)

        a8, cache8 = conv_forward_fast(a7, W7, b7, conv_param)
        bn4, cache_bn4 = spatial_batchnorm_forward(a8, gamma7, beta7, self.bn_params[6])
        a9, cache9 = relu_forward(bn4)
        cd3, cache_cd3 = dropout_forward(a9, self.dropout_param[1])     # convdrop5  0.4
        a10, cache10 = conv_forward_fast(cd3, W8, b8, conv_param)
        bn5, cache_bn5 = spatial_batchnorm_forward(a10, gamma8, beta8, self.bn_params[7])
        a11, cache11 = relu_forward(bn5)
        cd4, cache_cd4 = dropout_forward(a11, self.dropout_param[1])    # convdrop6  0.4
        a12, cache12 = conv_forward_fast(cd4, W9, b9, conv_param)
        bn6, cache_bn6 = spatial_batchnorm_forward(a12, gamma9, beta9, self.bn_params[8])
        a13, cache13 = relu_forward(bn6)
        a14, cache14 = max_pool_forward_fast(a13, pool_param)

        a15, cache15 = conv_forward_fast(a14, W10, b10, conv_param)
        bn7, cache_bn7 = spatial_batchnorm_forward(a15, gamma10, beta10, self.bn_params[9])
        a16, cache16 = relu_forward(bn7)
        cd5, cache_cd5 = dropout_forward(a16, self.dropout_param[1])    # convdrop7  0.4
        a17, cache17 = conv_forward_fast(cd5, W11, b11, conv_param)
        bn8, cache_bn8 = spatial_batchnorm_forward(a17, gamma11, beta11, self.bn_params[10])
        a18, cache18 = relu_forward(bn8)
        cd6, cache_cd6 = dropout_forward(a18, self.dropout_param[1])    # convdrop8  0.4
        a19, cache19 = conv_forward_fast(cd6, W12, b12, conv_param)
        bn9, cache_bn9 = spatial_batchnorm_forward(a19, gamma12, beta12, self.bn_params[11])
        ...
        a22, cache22 = affine_forward(out1, W13, b13)
        bn10, cache_bn10 = batchnorm_forward(a22, gamma13, beta13, self.bn_params[12])
        a23, cache23 = relu_forward(bn10)
        out2, cache_drop2 = dropout_forward(a23, self.dropout_param[2])    # dropout 0.5
        scores, cache24 = affine_forward(out2, W14, b14)
        ...
        dout2, dW14, db14 = affine_backward(dscores, cache24)
        da23 = dropout_backward(dout2, cache_drop2)             # dropout
        dbn10 = relu_backward(da23, cache23)
        da22, dgamma13, dbeta13 = batchnorm_backward(dbn10, cache_bn10)
        dout1, dW13, db13 = affine_backward(da22, cache22)
        da21 = dropout_backward(dout1, cache_drop1)             # dropout

        da20 = max_pool_backward_fast(da21, cache21)
        dbn9 = relu_backward(da20, cache20)
        da19, dgamma12, dbeta12 = spatial_batchnorm_backward(dbn9, cache_bn9)
        dcd6, dW12, db12 = conv_backward_fast(da19, cache19)
        da18 = dropout_backward(dcd6, cache_cd6)                # dconvdrop8  0.4
        dbn8 = relu_backward(da18, cache18)
        da17, dgamma11, dbeta11 = spatial_batchnorm_backward(dbn8, cache_bn8)
        dcd5, dW11, db11 = conv_backward_fast(da17, cache17)
        da16 = dropout_backward(dcd5, cache_cd5)                # dconvdrop7  0.4
        dbn7 = relu_backward(da16, cache16)
        da15, dgamma10, dbeta10 = spatial_batchnorm_backward(dbn7, cache_bn7)
        da14, dW10, db10 = conv_backward_fast(da15, cache15)

        da13 = max_pool_backward_fast(da14, cache14)
        dbn6 = relu_backward(da13, cache13)
        da12, dgamma9, dbeta9 = spatial_batchnorm_backward(dbn6, cache_bn6)
        dcd4, dW9, db9 = conv_backward_fast(da12, cache12)
        da11 = dropout_backward(dcd4, cache_cd4)                # dconvdrop6  0.4
        dbn5 = relu_backward(da11, cache11)
        da10, dgamma8, dbeta8 = spatial_batchnorm_backward(dbn5, cache_bn5)
        dcd3, dW8, db8 = conv_backward_fast(da10, cache10)
        da9 = dropout_backward(dcd3, cache_cd3)                 # dconvdrop5  0.4
        dbn4 = relu_backward(da9, cache9)
        da8, dgamma7, dbeta7 = spatial_batchnorm_backward(dbn4, cache_bn4)
        da7, dW7, db7 = conv_backward_fast(da8, cache8)

        daf4 = max_pool_backward_fast(da7, cache7)
        dbnf2 = relu_backward(daf4, cachef4)
        daf3, dgamma6, dbeta6 = spatial_batchnorm_backward(dbnf2, cache_bnf2)
        dcdf, dW6, db6 = conv_backward_fast(daf3, cachef3)
        da6 = dropout_backward(dcdf, cache_cdf)                 # dconvdrop4  0.4
        dbn3 = relu_backward(da6, cache6)
        da5, dgamma5, dbeta5 = spatial_batchnorm_backward(dbn3, cache_bn3)
        dcd2, dW5, db5 = conv_backward_fast(da5, cache5)
        daf2 = dropout_backward(dcd2, cache_cd2)                 # dconvdrop3  0.4
        dbnf = relu_backward(daf2, cachef2)
        daf1, dgamma4, dbeta4 = spatial_batchnorm_backward(dbnf, cache_bnf)
        daf, dW4, db4 = conv_backward_fast(daf1, cachef1)

        daf5 = max_pool_backward_fast(daf, cachef)
        dbnfs = relu_backward(daf5, cachef5)
        dafs, dgamma3, dbeta3 = spatial_batchnorm_backward(dbnfs, cache_bnfs)
        dcds, dW3, db3 = conv_backward_fast(dafs, cachefs)
        da4 = dropout_backward(dcds, cache_cds)                 # dconvdrop2  0.4
        ...
        dW13 += self.reg * W13
        dW14 += self.reg * W14
        reg_loss = 0.5 * self.reg * sum(np.sum(W * W) for W in [W1, W2, W3, W4, W5, W6, W7, W8, W9, W10, W11, W12, W13, W14])
        ...
        grads['gamma13'], grads['beta13'] = dgamma13, dbeta13
