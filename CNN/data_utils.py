...
def load_CIFAR_batch_100(filename):
    """ load single batch of cifar """
    with open(filename, 'rb') as f:
        datadict = pickle.load(f)
        #print datadict['fine_labels']
        X = datadict['data']
...
def load_CIFAR100(ROOT):
    """ load all of cifar """
    Xtr, Ytr = load_CIFAR_batch_100(os.path.join(ROOT, 'train'))
    Xte, Yte = load_CIFAR_batch_100(os.path.join(ROOT, 'test'))
    return Xtr, Ytr, Xte, Yte

def get_CIFAR10_data(num_training=49000, num_validation=1000, num_test=10000, data_aug='NO'):
    """
    ...
    X_val = X_val.transpose(0, 3, 1, 2).copy()            # ( 1000, 3, 32, 32)
    ...
    if data_aug == 'YES':
        ...
        x_last = np.zeros((2*N, C, H, W))
        for n in xrange(N):
            img = x[n, :, :, :]
            for i in xrange(H):
                for j in xrange(W):
                    x_da[i, W-1-j] = img[i, j]
                    x_aug[n, :, :, :] = x_da
...
def get_CIFAR100_data(num_training=49000, num_validation=1000, num_test=10000, data_aug='NO'):
    """
    ...
    cifar100_dir = 'E:/PycharmProjects/ML/CS231n/cifar-100-python'
